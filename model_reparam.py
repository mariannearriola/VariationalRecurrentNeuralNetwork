...
class VRNN(nn.Module):
	...
	def forward(self, x):
		...
		h = Variable(torch.zeros(self.n_layers, x.size(1), self.h_dim)).cuda(3)
		for t in range(x.size(0)):
			...
			if enc_mean_t == None and enc_std_t == None:
				enc_mean_t = self.enc_mean(enc_t)
				enc_std_t = self.enc_std(enc_t)
				z_t = self._reparameterized_sample(enc_mean_t, enc_std_t, enc_mean_t, enc_std_t)
			else:
				z_t = self._reparameterized_sample(enc_mean_t, enc_std_t, enc_mean_t, enc_std_t)
				enc_mean_t = self.enc_mean(enc_t)
				enc_std_t = self.enc_std(enc_t)
	...
	def sample(self, seq_len):
		...
		prior_mean_t,prior_std_t=None,None
